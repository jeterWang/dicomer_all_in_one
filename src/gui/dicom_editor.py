...
class DicomEditor(QWidget):
        ...
        for root, _, files in os.walk(self.dicom_dir):
            ...
            for file in files:
                if file.lower().endswith(('.dcm', '.dicom', '.ima')) or '.' not in file:
                    try:
                        # 相对路径（用于显示）
                        rel_path = os.path.relpath(root, self.dicom_dir)
                        if rel_path == '.':
                            rel_path = ''
                            
                        # 创建根目录项目（如果尚不存在）
                        if rel_path and not root_item:
                            root_item = QTreeWidgetItem(self.file_tree, [rel_path])
                            
                        # 创建文件项目
                        file_path = os.path.join(root, file)
                        if root_item:
                            item = QTreeWidgetItem(root_item, [file])
                        else:
                            item = QTreeWidgetItem(self.file_tree, [file])
    ...
    def on_file_selected(self, item, column):
        ...
        try:
            ...
            self.current_dataset = pydicom.dcmread(file_path, force=True)
    ...
    def display_attributes(self):
        ...
        for i, elem in enumerate(self.current_dataset):
            ...
            # 显示标签
            tag_item = QTableWidgetItem(f"({elem.tag.group:04x},{elem.tag.element:04x})")
            ...
            try:
                ...
                value_item = QTableWidgetItem(f"<无法显示: {str(e)}>")
                self.attr_table.setItem(i, 3, value_item)
    ...
    def save_changes(self):
        ...
        try:
            # 获取元素的VR
            element = self.current_dataset[tag]
            ...
            if vr in ['DS', 'FL', 'FD']:
                try:
                    ...
                    QMessageBox.warning(self, "警告", f"无法将 '{new_value}' 转换为浮点数！")
                    return
                ...
                try:
                    ...
                    QMessageBox.warning(self, "警告", f"无法将 '{new_value}' 转换为整数！")
                    return
            ...
            # 保存DICOM文件
            self.current_dataset.save_as(self.current_file)
    ...
    def batch_edit(self):
        ...
        try:
                    ...
                    if tag in ds:
                        ...
                        if vr in ['DS', 'FL', 'FD']:
                            try:
                                ...
                                error_files.append(f"{file_path} - 无法将值转换为浮点数")
                                continue
                            ...
                            try:
                                ...
                                error_files.append(f"{file_path} - 无法将值转换为整数")
                                continue
            ...
            if self.current_dataset and self.current_file:
                self.current_dataset = pydicom.dcmread(self.current_file, force=True)
                self.display_attributes()
